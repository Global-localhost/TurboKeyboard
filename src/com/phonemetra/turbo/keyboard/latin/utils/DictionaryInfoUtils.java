...
public class DictionaryInfoUtils {
	...
	public static class DictionaryInfo {
		...
		public ContentValues toContentValues() {
			...
			values.put(DATE_COLUMN,
					TimeUnit.MILLISECONDS.toSeconds(mModifiedTimeMillis));
		...
		public String toString() {
			return "DictionaryInfo : Id = '" + mId + "' : Locale=" + mLocale
					+ " : Version=" + mVersion;
		}
	}
	 ...
	 *
	 * This basically only accepts ascii letters and numbers, and rejects
	 * everything else.
	 */
	...
	/**
	 * Escapes a string for any characters that may be suspicious for a file or
	 * directory name.
	 *
	 * Concretely this does a sort of URL-encoding except it will encode
	 * everything that's not alphanumeric or underscore. (true URL-encoding
	 * leaves alone characters like '*', which we cannot allow here)
	 */
	...
	public static String replaceFileNameDangerousCharacters(final String name) {
		...
		for (int i = 0; i < nameLength; i = name.offsetByCodePoints(i, 1)) {
			...
			if (DictionaryInfoUtils.isFileNameCharacter(codePoint)) {
				...
				sb.append(String.format((Locale) null, "%%%1$0"
						+ MAX_HEX_DIGITS_FOR_CODEPOINT + "x", codePoint));
			}
		}
		return sb.toString();
	}
	...
	/**
	 * Reverse escaping done by
	 * {@link #replaceFileNameDangerousCharacters(String)}.
	 */
	@Nonnull
		...
		for (int i = 0; i < fnameLength; i = fname.offsetByCodePoints(i, 1)) {
			...
			if ('%' != codePoint) {
				...
				final int encodedCodePoint = Integer.parseInt(
						fname.substring(i + 1, i + 1
								+ MAX_HEX_DIGITS_FOR_CODEPOINT), 16);
	...
	/**
	 * Helper method to the list of cache directories, one for each distinct
	 * locale.
	 */
	public static File[] getCachedDirectoryList(final Context context) {
		return new File(DictionaryInfoUtils.getWordListCacheDirectory(context))
				.listFiles();
	}

	public static File[] getStagingDirectoryList(final Context context) {
		return new File(
				DictionaryInfoUtils.getWordListStagingDirectory(context))
				.listFiles();
	}
	 ...
	 * 
	 * @return The category as a string or null if it can't be found in the file
	 *         name.
	 */
	@Nullable
	public static String getCategoryFromFileName(@Nonnull final String fileName) {
		final String id = getWordListIdFromFileName(fileName);
		final String[] idArray = id
				.split(BinaryDictionaryGetter.ID_CATEGORY_SEPARATOR);
		// An id is supposed to be in format category:locale, so splitting on
		// the separator
	 ...
	 */
	public static String getCacheDirectoryForLocale(final String locale,
			final Context context) {
		final String relativeDirectoryName = replaceFileNameDangerousCharacters(locale);
		final String absoluteDirectoryName = getWordListCacheDirectory(context)
				+ File.separator + relativeDirectoryName;
	 ...
	 *
	 * In the current implementation the file name returned will always be
	 * unique for any id/locale pair, but please do not expect that the id can
	 * be the same for different dictionaries with different locales. An id
	 * should be unique for any dictionary. The file name is pretty much an
	 * URL-encoded version of the id inside a directory named like the locale,
	 * except it will also escape characters that look dangerous to some file
	 * systems.
	 * 
	 * @param id
	 *            the id of the dictionary for which to get a file name
	 * @param locale
	 *            the locale for which to get the file name as a string
	 * @param context
	 *            the context to use for getting the directory
	 * @return the name of the file to be created
	 */
	public static String getCacheFileName(String id, String locale,
			Context context) {
	...
	public static String getStagingFileName(String id, String locale,
			Context context) {
	...
	public static void moveStagingFilesIfExists(Context context) {
		final File[] stagingFiles = DictionaryInfoUtils
				.getStagingDirectoryList(context);
		if (stagingFiles != null && stagingFiles.length > 0) {
			for (final File stagingFile : stagingFiles) {
				...
				final String[] localeAndFileId = fileName
						.split(TEMP_DICT_FILE_SUB);
				...
				final String cacheDirectoryForLocale = getCacheDirectoryForLocale(
						locale, context);
				final String cacheFilename = cacheDirectoryForLocale
	 ...
	 * 
	 * @param context
	 *            the context on which to check resources.
	 * @param locale
	 *            the locale to check for.
	 * @return whether a (non-placeholder) dictionary is available or not.
	 */
	public static boolean isDictionaryAvailable(final Context context,
			final Locale locale) {
	 ...
	 */
	public static int getMainDictionaryResourceIdIfAvailableForLocale(
			final Resources res, final Locale locale) {
		int resId;
		// Try to find main_language_country dictionary.
		if (!locale.getCountry().isEmpty()) {
			final String dictLanguageCountry = MAIN_DICT_PREFIX
					+ locale.toString().toLowerCase(Locale.ROOT)
					+ DECODER_DICT_SUFFIX;
			if ((resId = res.getIdentifier(dictLanguageCountry, "raw",
					RESOURCE_PACKAGE_NAME)) != 0) {
				return resId;
			}
		}
	 ...
	 * @return main dictionary resource id
	 */
	public static int getMainDictionaryResourceId(final Resources res,
			final Locale locale) {
		int resourceId = getMainDictionaryResourceIdIfAvailableForLocale(res,
				locale);
		if (0 != resourceId) {
			return resourceId;
		}
		return res.getIdentifier(DEFAULT_MAIN_DICT
				+ DecoderSpecificConstants.DECODER_DICT_SUFFIX, "raw",
	 ...
	 *
	 * Word lists stored in Android Keyboard's resources are referred to as the
	 * "main" word lists. Since they can be updated like any other list, we need
	 * to assign a unique ID to them. This ID is just the name of the language
	 * (locale-wise) they are for, and this method returns this ID.
	 */
	public static String getMainDictId(@Nonnull final Locale locale) {
		// This works because we don't include by default different dictionaries
		// for
		// different countries. This actually needs to return the id that we
		// would
	...
	public static DictionaryHeader getDictionaryFileHeaderOrNull(
			final File file, final long offset, final long length) {
		try {
			final DictionaryHeader header = BinaryDictionaryUtils
					.getHeaderWithOffsetAndLength(file, offset, length);
			return header;
	 ...
	 *
	 * @param fileAddress
	 *            the asset dictionary file address.
	 * @param locale
	 *            Locale for this file.
	...
	private static DictionaryInfo createDictionaryInfoFromFileAddress(
		...
		final int version = DictionaryHeaderUtils
				.getContentVersion(fileAddress);
		final String description = SubtypeLocaleUtils
				.getSubtypeLocaleDisplayName(locale.toString());
		// Do not store the filename on db as it will try to move the filename
		// from db to the
		// cached directory. If the filename is already in cached directory,
		// this is not
		// necessary.
		final String filenameToStoreOnDb = null;
		return new DictionaryInfo(id, locale, description, filenameToStoreOnDb,
				fileAddress.mLength,
				new File(fileAddress.mFilename).lastModified(), version);
	}

	/**
	 * Returns the information of the dictionary for the given
	 * {@link AssetFileAddress}. If the file is corrupted or a pre-fava file,
	 * then the file gets deleted and the null value is returned.
	 */
	@Nullable
	private static DictionaryInfo createDictionaryInfoForUnCachedFile(
		...
		return new DictionaryInfo(id, locale, description, filenameToStoreOnDb,
				fileAddress.mLength, unCachedFile.lastModified(), version);
	}
	...
	private static DictionaryInfo createDictionaryInfoFromLocale(Locale locale) {
		...
		final String description = SubtypeLocaleUtils
				.getSubtypeLocaleDisplayName(locale.toString());
		return new DictionaryInfo(id, locale, description, null, 0L, 0L,
				version);
	}

	private static void addOrUpdateDictInfo(
			final ArrayList<DictionaryInfo> dictList,
	...
	public static ArrayList<DictionaryInfo> getCurrentDictionaryFileNameAndVersionInfo(
		...
		if (null != directoryList) {
			for (final File directory : directoryList) {
				final String localeString = getWordListIdFromFileName(directory
						.getName());
				final File[] dicts = BinaryDictionaryGetter.getCachedWordLists(
						localeString, context);
				for (final File dict : dicts) {
					final String wordListId = getWordListIdFromFileName(dict
							.getName());
					...
					final AssetFileAddress fileAddress = AssetFileAddress
							.makeFromFile(dict);
...
//		}
		...
		for (InputMethodSubtype subtype : enabledSubtypes) {
			Locale locale = LocaleUtils.constructLocaleFromString(subtype
					.getLocale());
	...
	public static boolean looksValidForDictionaryInsertion(
		...
		// We reject strings entirely comprised of digits to avoid using PIN
		// codes or credit
		// card numbers. It would come in handy for word prediction though; a
		// good example is
		// when writing one's address where the street number is usually quite
		// discriminative,
