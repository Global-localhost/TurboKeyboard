...
import com.phonemetra.turbo.keyboard.internal.BatchInputArbiter.BatchInputArbiterListener;
import com.phonemetra.turbo.keyboard.internal.BogusMoveEventDetector;
import com.phonemetra.turbo.keyboard.internal.DrawingProxy;
import com.phonemetra.turbo.keyboard.internal.GestureEnabler;
import com.phonemetra.turbo.keyboard.internal.GestureStrokeDrawingParams;
import com.phonemetra.turbo.keyboard.internal.GestureStrokeDrawingPoints;
import com.phonemetra.turbo.keyboard.internal.GestureStrokeRecognitionParams;
import com.phonemetra.turbo.keyboard.internal.PointerTrackerQueue;
import com.phonemetra.turbo.keyboard.internal.TimerProxy;
import com.phonemetra.turbo.keyboard.internal.TypingTimeRecorder;
...
public final class PointerTracker implements PointerTrackerQueue.Element,
	...
	static final class PointerTrackerParams {
		...
		public PointerTrackerParams(final TypedArray mainKeyboardViewAttr) {
			mKeySelectionByDraggingFinger = mainKeyboardViewAttr.getBoolean(
					R.styleable.MainKeyboardView_keySelectionByDraggingFinger,
					false);
			mTouchNoiseThresholdTime = mainKeyboardViewAttr.getInt(
					R.styleable.MainKeyboardView_touchNoiseThresholdTime, 0);
			mTouchNoiseThresholdDistance = mainKeyboardViewAttr
					.getDimensionPixelSize(
							R.styleable.MainKeyboardView_touchNoiseThresholdDistance,
							0);
			mSuppressKeyPreviewAfterBatchInputDuration = mainKeyboardViewAttr
					.getInt(R.styleable.MainKeyboardView_suppressKeyPreviewAfterBatchInputDuration,
							0);
	...
	// true if this pointer is no longer triggering any action because it has
	// been canceled.
	private boolean mIsTrackingForActionDisabled;

	// the more keys panel currently being shown. equals null if no panel is
	// active.
	...
	// true if this pointer is sliding from a modifier key and in the sliding
	// key input mode,
	...
	// TODO: Add PointerTrackerFactory singleton and move some class static
	// methods into it.
	public static void init(final TypedArray mainKeyboardViewAttr,
			final TimerProxy timerProxy, final DrawingProxy drawingProxy) {
		...
		sNeedsPhantomSuddenMoveEventHack = Boolean.parseBoolean(ResourceUtils
				.getDeviceOverrideValue(res,
	...
	public static void setKeyboardActionListener(
	...
	private PointerTracker(final int id) {
		mPointerId = id;
		mBatchInputArbiter = new BatchInputArbiter(id,
	...
	private boolean callListenerOnPressAndCheckKeyboardLayoutChange(
		...
		// While gesture input is going on, this method should be a
		// no-operation. But when gesture
		...
		// <code>mIsTrackingForActionDisabled</code> should also be taken
		// account of.
		if (sInGesture || mIsDetectingGesture || mIsTrackingForActionDisabled) {
			return false;
		}
		final boolean ignoreModifierKey = mIsInDraggingFinger
				&& key.isModifier();
		...
		if (key.isEnabled()) {
			sListener.onPressKey(key.getCode(), repeatCount,
	...
	private void callListenerOnCodeInput(final Key key, final int primaryCode,
			final int x, final int y, final long eventTime,
			final boolean isKeyRepeat) {
		final boolean ignoreModifierKey = mIsInDraggingFinger
				&& key.isModifier();
		final boolean altersCode = key.altCodeWhileTyping()
				&& sTimerProxy.isTypingState();
			...
			if (code == Constants.CODE_OUTPUT_TEXT) {
				...
				if (mKeyboard.hasProximityCharsCorrection(code)) {
					...
					sListener.onCodeInput(code, Constants.NOT_A_COORDINATE,
							Constants.NOT_A_COORDINATE, isKeyRepeat);
				}
			}
		}
	}

	// Note that we need primaryCode argument because the keyboard may be in
	// shifted state and the
	// primaryCode is different from {@link Key#mKeyCode}.
	...
	private void setKeyDetectorInner(final KeyDetector keyDetector) {
		...
		mBatchInputArbiter.setKeyboardGeometry(keyWidth,
				mKeyboard.mOccupiedHeight);
		// Keep {@link #mCurrentKey} that comes from previous keyboard. The key
		// preview of
		// {@link #mCurrentKey} will be dismissed by
		// {@setReleasedKeyGraphics(Key)} via
	...
	private void setReleasedKeyGraphics(@Nullable final Key key,
		...
		if (key.isShift()) {
			for (final Key shiftKey : mKeyboard.mShiftKeys) {
				if (shiftKey != key) {
					sDrawingProxy
							.onKeyReleased(shiftKey, false /* withAnimation */);
				}
			}
		}
	...
	private void setPressedKeyGraphics(@Nullable final Key key,
		...
		final boolean noKeyPreview = sInGesture
				|| needsToSuppressKeyPreviewPopup(eventTime);
		...
		if (key.isShift()) {
			for (final Key shiftKey : mKeyboard.mShiftKeys) {
				if (shiftKey != key) {
					sDrawingProxy
							.onKeyPressed(shiftKey, false /* withPreview */);
				}
			}
		}
	...
	private static int getDistance(final int x1, final int y1, final int x2,
			final int y2) {
	...
	private void showGestureTrail() {
		...
		// pointer/finger on the screen.
		sDrawingProxy
				.showGestureTrail(this, isOldestTrackerInQueue() /* showsFloatingPreviewText */);
	}

	public void updateBatchInputByTimer(final long syntheticMoveEventTime) {
		mBatchInputArbiter
				.updateBatchInputByTimer(syntheticMoveEventTime, this);
	}

	// Implements {@link BatchInputArbiterListener}.
	@Override
	public void onUpdateBatchInput(final InputPointers aggregatedPointers,
			final long eventTime) {
	...
	// Implements {@link BatchInputArbiterListener}.
	@Override
	public void onEndBatchInput(final InputPointers aggregatedPointers,
			final long eventTime) {
	...
	public void processMotionEvent(final MotionEvent me,
		...
		if (action == MotionEvent.ACTION_MOVE) {
			// When this pointer is the only active pointer and is showing a
			// more keys panel,
			// we should ignore other pointers' motion event.
			final boolean shouldIgnoreOtherPointers = isShowingMoreKeysPanel()
					&& getActivePointerTrackerCount() == 1;
	...
	private void onDownEvent(final int x, final int y, final long eventTime,
		...
		// A gesture should start only from a non-modifier key. Note that the
		// gesture detection is
		// disabled when the key is repeating.
		mIsDetectingGesture = (mKeyboard != null)
				&& mKeyboard.mId.isAlphabetKeyboard() && key != null
				&& !key.isModifier();
		if (mIsDetectingGesture) {
			...
			mGestureStrokeDrawingPoints.onDownEvent(x, y,
					mBatchInputArbiter.getElapsedTimeSinceFirstDown(eventTime));
		}
	}
	...
	private void onDownEventInternal(final int x, final int y,
		...
		// Key selection by dragging finger is allowed when 1) key selection by
		// dragging finger is
		// enabled by configuration, 2) this pointer starts dragging from
		// modifier key, or 3) this
		// pointer's KeyDetector always allows key selection by dragging finger,
		// such as
	...
	private void onGestureMoveEvent(final int x, final int y,
			final long eventTime, final boolean isMajorEvent, final Key key) {
		if (!mIsDetectingGesture) {
			return;
		}
		final boolean onValidArea = mBatchInputArbiter.addMoveEventPoint(x, y,
				eventTime, isMajorEvent, this);
		// If the move event goes out from valid batch input area, cancel batch
		// input.
		if (!onValidArea) {
			cancelBatchInput();
			return;
		}
		mGestureStrokeDrawingPoints.onMoveEvent(x, y,
				mBatchInputArbiter.getElapsedTimeSinceFirstDown(eventTime));
		// If the MoreKeysPanel is showing then do not attempt to enter gesture
		// mode. However,
		// the gestured touch points are still being recorded in case the panel
		// is dismissed.
		if (isShowingMoreKeysPanel()) {
			return;
		}
	...
	private void onMoveEvent(final int x, final int y, final long eventTime,
		...
		if (sGestureEnabler.shouldHandleGesture() && me != null) {
			...
			for (int h = 0; h < historicalSize; h++) {
				final int historicalX = (int) me
						.getHistoricalX(pointerIndex, h);
				final int historicalY = (int) me
						.getHistoricalY(pointerIndex, h);
	...
	private void processDraggingFingerInToNewKey(final Key newKey, final int x,
			final int y, final long eventTime) {
		// This onPress call may have changed keyboard layout. Those cases are
		// detected
		// at {@link #setKeyboard}. In those cases, we should update key
		// according
	...
	private void processPhantomSuddenMoveHack(final Key key, final int x,
			final int y, final long eventTime, final Key oldKey,
	...
	private void processProximateBogusDownMoveUpEventHack(final Key key,
			final int x, final int y, final long eventTime, final Key oldKey,
			final int lastX, final int lastY) {
	...
	private void dragFingerFromOldKeyToNewKey(final Key key, final int x,
			final int y, final long eventTime, final Key oldKey,
			final int lastX, final int lastY) {
		// The pointer has been slid in to the new key from the previous key, we
		// must call
		// onRelease() first to notify that the previous key has been released,
		// then call
		...
		// HACK: On some devices, quick successive touches may be reported as a
		// sudden move by
		// touch panel firmware. This hack detects such cases and translates the
		// move event to
		...
		// TODO: Should find a way to balance gesture detection and this hack.
			...
			processPhantomSuddenMoveHack(key, x, y, eventTime, oldKey, lastX,
					lastY);
		}
		// HACK: On some devices, quick successive proximate touches may be
		// reported as a bogus
		// down-move-up event by touch panel firmware. This hack detects such
		// cases and breaks
		// these events into separate up and down events.
			...
			processProximateBogusDownMoveUpEventHack(key, x, y, eventTime,
					oldKey, lastX, lastY);
		}
		// HACK: If there are currently multiple touches, register the key even
		// if the finger
		// slides off the key. This defends against noise from some touch panels
		// when there are
	...
	private void dragFingerOutFromOldKey(final Key oldKey, final int x,
			final int y) {
		// The pointer has been slid out from the previous key, we must call
		// onRelease() to
	...
	private void onMoveEventInternal(final int x, final int y,
			final long eventTime) {
		...
		if (newKey != null) {
			if (oldKey != null
					&& isMajorEnoughMoveToBeOnNewKey(x, y, eventTime, newKey)) {
	...
	private void onUpEvent(final int x, final int y, final long eventTime) {
		...
		if (!sInGesture) {
			if (mCurrentKey != null && mCurrentKey.isModifier()) {
				...
				sPointerTrackerQueue.releaseAllPointersOlderThan(this,
						eventTime);
			}
		}
		onUpEventInternal(x, y, eventTime);
	...
	private void onUpEventInternal(final int x, final int y,
		...
		if (sInGesture) {
			...
			if (mBatchInputArbiter.mayEndBatchInput(eventTime,
					getActivePointerTrackerCount(), this)) {
				sInGesture = false;
			}
			showGestureTrail();
			return;
	...
	public void onLongPressed() {
		...
		if (key.hasNoPanelAutoMoreKey()) {
			...
			sListener
					.onPressKey(moreKeyCode, 0 /* repeatCont */, true /* isSinglePointer */);
		...
		final MoreKeysPanel moreKeysPanel = sDrawingProxy.showMoreKeysKeyboard(
				key, this);
	...
	private boolean isMajorEnoughMoveToBeOnNewKey(final int x, final int y,
			final long eventTime, final Key newKey) {
		...
		final int keyHysteresisDistanceSquared = mKeyDetector
				.getKeyHysteresisDistanceSquared(mIsInSlidingKeyInput);
		final int distanceFromKeyEdgeSquared = curKey.squaredDistanceToEdge(x,
				y);
	...
	private void startLongPressTimer(final Key key) {
		// Note that we need to cancel all active long press shift key timers if
		// any whenever we
		...
		if (!key.isLongPressEnabled())
			return;
		// Caveat: Please note that isLongPressEnabled() can be true even if the
		// current key
		// doesn't have its more keys. (e.g. spacebar, globe key) If we are in
		// the dragging finger
		// mode, we will disable long press timer of such key.
		// We always need to start the long press timer if the key has its more
		// keys regardless of
		// whether or not we are in the dragging finger mode.
		if (mIsInDraggingFinger && key.getMoreKeys() == null)
			return;
	...
	private int getLongPressTimeout(final int code) {
		...
		if (mIsInSlidingKeyInput) {
			// We use longer timeout for sliding finger input started from the
			// modifier key.
	...
	public void onKeyRepeat(final int code, final int repeatCount) {
		...
		callListenerOnCodeInput(key, code, mKeyX, mKeyY,
				SystemClock.uptimeMillis(), true /* isKeyRepeat */);
	}

	private void startKeyRepeatTimer(final int repeatCount) {
		final int delay = (repeatCount == 1) ? sParams.mKeyRepeatStartTimeout
				: sParams.mKeyRepeatInterval;
