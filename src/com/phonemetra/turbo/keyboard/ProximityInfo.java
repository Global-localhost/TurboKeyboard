...
import com.phonemetra.turbo.keyboard.internal.TouchPositionCorrection;
...
public class ProximityInfo extends Activity {
	...
	ProximityInfo(final int gridWidth, final int gridHeight,
	...
	private static native long setProximityInfoNative(int displayWidth,
			int displayHeight, int gridWidth, int gridHeight,
			int mostCommonKeyWidth, int mostCommonKeyHeight,
			int[] proximityCharsArray, int keyCount, int[] keyXCoordinates,
			int[] keyYCoordinates, int[] keyWidths, int[] keyHeights,
			int[] keyCharCodes, float[] sweetSpotCenterXs,
			float[] sweetSpotCenterYs, float[] sweetSpotRadii);
	
	private static native long initProximityInfoNative(long nativeProximityInfo);
	...
	private long createNativeProximityInfo(
		...
		final int[] proximityCharsArray = new int[mGridSize
		...
		if (touchPositionCorrection.isValid()) {
			...
			for (int infoIndex = 0, keyIndex = 0; keyIndex < sortedKeys.size(); keyIndex++) {
				...
				if (row < rows) {
					...
					final float hitBoxDiagonal = (float) Math.hypot(
							hitBoxWidth, hitBoxHeight);
					sweetSpotCenterXs[infoIndex] += touchPositionCorrection
							.getX(row) * hitBoxWidth;
					sweetSpotCenterYs[infoIndex] += touchPositionCorrection
							.getY(row) * hitBoxHeight;
					sweetSpotRadii[infoIndex] = touchPositionCorrection
							.getRadius(row) * hitBoxDiagonal;
				}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (mTouchPositionCorrection!= null) {
			mNativeProximityInfo = initProximityInfoNative(createNativeProximityInfo(mTouchPositionCorrection));
		}
	...
	private void computeNearestNeighbors() {
		...
		for (final Key key : mSortedKeys) {
			 ...
			 * 
			 * We want to compute the distance for keys that are in the cells
			 * that are close enough to the key border, as this method is
			 * performance-critical. These keys are represented with 'star'
			 * background on the diagram below. Let's consider the Y case first.
			 * 
			 * We want to select the cells which center falls between the top of
			 * the key minus the threshold, and the bottom of the key plus the
			 * threshold. topPixelWithinThreshold is key.mY - threshold, and
			 * bottomPixelWithinThreshold is key.mY + key.mHeight + threshold.
			 * 
			 * Then we need to compute the center of the top row that we need to
			 * evaluate, as we'll iterate from there.
			 * 
			 * (0,0)----> x | .-------------------------------------------. | |
			 * | | | | | | | | | | | |
			 * |---+---+---+---+---+---+---+---+---+---+---| .- top of top cell
			 ...
			 * is stored in yStart.
			 * 
			 * Since we only want to go up to bottomPixelWithinThreshold, and we
			 * only iterate on the center of the keys, we can stop as soon as
			 * the y value exceeds bottomPixelThreshold, so we don't have to
			 * align this on the center of the key. Hence, we don't need a
			 * separate value for bottomPixelWithinThreshold and call this yEnd
			 * right away.
			 */
			...
			final int yMiddleOfTopCell = topPixelWithinThreshold - yDeltaToGrid
					+ halfCellHeight;
			final int yStart = Math.max(halfCellHeight, yMiddleOfTopCell
					+ (yDeltaToGrid <= halfCellHeight ? 0 : mCellHeight));
			final int yEnd = Math.min(lastPixelYCoordinate,
					keyY + key.getHeight() + threshold);

			final int leftPixelWithinThreshold = keyX - threshold;
			final int xDeltaToGrid = leftPixelWithinThreshold % mCellWidth;
			final int xMiddleOfLeftCell = leftPixelWithinThreshold
					- xDeltaToGrid + halfCellWidth;
			...
			int baseIndexOfCurrentRow = (yStart / mCellHeight) * mGridWidth
					+ (xStart / mCellWidth);
			for (int centerY = yStart; centerY <= yEnd; centerY += mCellHeight) {
				...
				for (int centerX = xStart; centerX <= xEnd; centerX += mCellWidth) {
					if (key.squaredDistanceToEdge(centerX, centerY) < thresholdSquared) {
						neighborsFlatBuffer[index * keyCount
								+ neighborCountPerCell[index]] = key;
		...
		for (int i = 0; i < gridSize; ++i) {
			...
			final ArrayList<Key> neighbors = new ArrayList<>(indexEnd
					- indexStart);
	...
	public void fillArrayWithNearestKeyCodes(final int x, final int y,
			final int primaryKeyCode, final int[] dest) {
