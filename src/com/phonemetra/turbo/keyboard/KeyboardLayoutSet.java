...
import static com.phonemetra.turbo.keyboard.latin.common.Constants.ImeOption.FORCE_ASCII;
import static com.phonemetra.turbo.keyboard.latin.common.Constants.ImeOption.NO_SETTINGS_KEY;
...
import com.phonemetra.turbo.keyboard.compat.EditorInfoCompatUtils;
...
import com.phonemetra.turbo.keyboard.internal.UniqueKeysCache;
...
import com.phonemetra.turbo.keyboard.latin.utils.XmlParseUtils;
...
public final class KeyboardLayoutSet {
	...
	@Nonnull
	private static final UniqueKeysCache sUniqueKeysCache = UniqueKeysCache
			.newInstance();
	private final static HashMap<InputMethodSubtype, Integer> sScriptIdsForSubtypes = new HashMap<>();

	@SuppressWarnings("serial")
	public static final class KeyboardLayoutSetException extends
			RuntimeException {
		public final KeyboardId mKeyboardId;

		public KeyboardLayoutSetException(final Throwable cause,
				final KeyboardId keyboardId) {
			super(cause);
	...
	public Keyboard getKeyboard(final int baseKeyboardLayoutSetElementId) {
		...
		ElementParams elementParams = mParams.mKeyboardLayoutSetElementIdToParamsMap
				.get(keyboardLayoutSetElementId);
		if (elementParams == null) {
			elementParams = mParams.mKeyboardLayoutSetElementIdToParamsMap
					.get(KeyboardId.ELEMENT_ALPHABET);
		}
		...
		final KeyboardId id = new KeyboardId(keyboardLayoutSetElementId,
				mParams);
		try {
	...
	@Nonnull
	private Keyboard getKeyboard(final ElementParams elementParams,
			final KeyboardId id) {
		...
		final KeyboardBuilder<KeyboardParams> builder = new KeyboardBuilder<>(
				mContext, new KeyboardParams(sUniqueKeysCache));
	...
	public static final class Builder {
		...
		public Builder(final Context context, @Nullable final EditorInfo ei) {
			...
			params.mIsPasswordField = InputTypeUtils
					.isPasswordInputType(editorInfo.inputType);
			params.mNoSettingsKey = InputAttributes.inPrivateImeOptions(
					mPackageName, NO_SETTINGS_KEY, editorInfo);
		}

		public Builder setKeyboardGeometry(final int keyboardWidth,
		...
		public Builder setSubtype(@Nonnull final RichInputMethodSubtype subtype) {
			final boolean asciiCapable = InputMethodSubtypeCompatUtils
					.isAsciiCapable(subtype);
			final boolean deprecatedForceAscii = InputAttributes
					.inPrivateImeOptions(mPackageName, FORCE_ASCII,
							mParams.mEditorInfo);
			final boolean forceAscii = EditorInfoCompatUtils
					.hasFlagForceAscii(mParams.mEditorInfo.imeOptions)
					|| deprecatedForceAscii;
			final RichInputMethodSubtype keyboardSubtype = (forceAscii && !asciiCapable) ? RichInputMethodSubtype
					.getNoLanguageSubtype() : subtype;
		...
		// Super redux version of reading the script ID for some subtype from
		// Xml.
		static int readScriptId(final Resources resources,
			...
			try {
				while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
					// Bovinate through the XML stupidly searching for
					// TAG_FEATURE, and read
		...
		private static int readScriptIdFromTagFeature(
				final Resources resources, final XmlPullParser parser)
				throws IOException, XmlPullParserException {
			final TypedArray featureAttr = resources.obtainAttributes(
					Xml.asAttributeSet(parser),
					R.styleable.KeyboardLayoutSet_Feature);
			try {
				final int scriptId = featureAttr.getInt(
		...
		public KeyboardLayoutSet build() {
			if (mParams.mSubtype == null)
				throw new RuntimeException(
						"KeyboardLayoutSet subtype is not specified");
			final int xmlId = getXmlId(mResources,
					mParams.mKeyboardLayoutSetName);
			try {
				parseKeyboardLayoutSet(mResources, xmlId);
			} catch (final IOException | XmlPullParserException e) {
				throw new RuntimeException(e.getMessage() + " in "
						+ mParams.mKeyboardLayoutSetName, e);
			}
			return new KeyboardLayoutSet(mContext, mParams);
		}

		private static int getXmlId(final Resources resources,
				final String keyboardLayoutSetName) {
			final String packageName = resources
					.getResourcePackageName(R.xml.keyboard_layout_set_qwerty);
			return resources.getIdentifier(keyboardLayoutSetName, "xml",
					packageName);
		}
				...
				while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
					...
					if (event == XmlPullParser.START_TAG) {
						...
						if (TAG_KEYBOARD_SET.equals(tag)) {
							...
							throw new XmlParseUtils.IllegalStartTag(parser,
									tag, TAG_KEYBOARD_SET);
						}
					}
				}
			} finally {
			...
			while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
				...
				if (event == XmlPullParser.START_TAG) {
					...
					if (TAG_ELEMENT.equals(tag)) {
						...
						mParams.mScriptId = readScriptIdFromTagFeature(
								mResources, parser);
					} else {
						throw new XmlParseUtils.IllegalStartTag(parser, tag,
								TAG_KEYBOARD_SET);
					}
					...
					throw new XmlParseUtils.IllegalEndTag(parser, tag,
							TAG_KEYBOARD_SET);
				}
			}
		}
		...
		private void parseKeyboardLayoutSetElement(final XmlPullParser parser)
			...
			try {
				XmlParseUtils.checkAttributeExists(a,
						...
						"elementName", TAG_ELEMENT, parser);
				XmlParseUtils.checkAttributeExists(a,
						R.styleable.KeyboardLayoutSet_Element_elementKeyboard,
						"elementKeyboard", TAG_ELEMENT, parser);
				...
				elementParams.mKeyboardXmlId = a.getResourceId(
						R.styleable.KeyboardLayoutSet_Element_elementKeyboard,
						0);
				elementParams.mProximityCharsCorrectionEnabled = a
						.getBoolean(
								R.styleable.KeyboardLayoutSet_Element_enableProximityCharsCorrection,
								false);
				elementParams.mSupportsSplitLayout = a
						.getBoolean(
								R.styleable.KeyboardLayoutSet_Element_supportsSplitLayout,
								false);
				elementParams.mAllowRedundantMoreKeys = a
						.getBoolean(
								R.styleable.KeyboardLayoutSet_Element_allowRedundantMoreKeys,
								true);
				mParams.mKeyboardLayoutSetElementIdToParamsMap.put(elementName,
						elementParams);
			} finally {
				a.recycle();
			}
